...
Nodes are connected via a [**Tailscale**](https://tailscale.com) mesh VPN. The k3s datastore is PostgreSQL (via kine) on `saraneth`. Tailscale advertises the home LAN (`192.168.1.0/24`) into the tailnet, so Pi-scheduled pods can reach home devices; Hetzner workers don&rsquo;t accept the route (it would break flannel VXLAN over their private network, see `hetzner/cloud-init-template.yaml`) and have no home-LAN reachability (#617).
